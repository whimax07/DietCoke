...
##############################################################################
    ...
    def ColourSectionBuilder(self, xPosIndex, BGColour, ABCol, white, startVal):
        ...
        ## User text imput
        # String Var is used to have somthing that can be passed to the callback
        strvar = StringVar()
        CText = Entry(cBox, bg = BGColour, fg = white, relief = 'groove', textvariable = strvar)
        CText.grid(column = 0, row = 1)
        # Set a call back when the text entry is clicked away from
        CText.bind("<FocusOut>", lambda event, slider = C, strvar = strvar: self.userRGB(event, slider, strvar))
        CText.bind("<Return>", lambda event, slider = C, strvar = strvar: self.userRGB(event, slider, strvar))
